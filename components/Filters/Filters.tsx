...
const Filters = () => {
	...
	const { filters, selectedFilters } = useAppSelector(store => store.filters)
	...
	useEffect(() => {
		...
		filters.map(f => {
			...
			request[f.slug] = f.content.filter(el => selectedFilters.includes(el))
			if (f.slug === 'price') {
				const priceFilter = selectedFilters.find(el => el.includes('Цена'))
				if (priceFilter) {
					request.price = {
